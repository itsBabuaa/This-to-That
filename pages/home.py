...
with c1:
    with st.container(border=True):
        ...
        st.page_link("pages/1_📝_Paste_Text.py", label="Open →", use_container_width=True)

with c2:
    with st.container(border=True):
        st.subheader("📁 Upload Files")
        st.write("Upload one or multiple files. Supports txt, json, csv, md, xml, html, yml and more.")
        st.page_link("pages/2_📁_Upload_Files.py", label="Open →", use_container_width=True)

with c3:
    with st.container(border=True):
        st.subheader("📋 Multi Text Blocks")
        st.write("Add multiple text blocks and translate them all at once. Great for batch snippets.")
        st.page_link("pages/3_📋_Multi_Text_Blocks.py", label="Open →", use_container_width=True)
